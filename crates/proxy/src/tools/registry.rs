...
pub(crate) fn tool_registry(
    ...
    let mut tools = match json!([
        ...
        },
        {
            ...
            "config": [
                {
                    "key": "WEB_SEARCH_PROXY_MODE",
                    "type": "segmented",
                    "labelKey": "webSearchProxyMode",
                    "label": "Network proxy",
                    "description": "Control whether web_search uses the system proxy. The default follows the system proxy.",
                    "defaultValue": "system",
                    "value": web_search_proxy_value(config.web_search_proxy),
                    "options": [
...
fn web_search_proxy_value(value: codeseex_core::WebSearchProxyMode) -> &'static str {
    match value {
...
mod tests {
    ...
    #[test]
    fn web_search_registry_exposes_proxy_mode_config() {
        ...
        assert_eq!(
            web_search.pointer("/config/0/key").and_then(Value::as_str),
            Some("WEB_SEARCH_PROXY_MODE")
        );
        assert_eq!(
            web_search
                .pointer("/config/0/value")
                .and_then(Value::as_str),
            Some("system")
        );
    }
